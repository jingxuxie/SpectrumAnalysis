# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 16:04:18 2019
...
def Channel2Energy(Spectrum0):
    Spectrum0_copy=Spectrum0.copy()#copy一下防止修改原变量
    x=[680,750,316,916,1016]
    y=[6.4,7.06,2.96,8.64,9.57]
    coef=np.polyfit(x,y,1)
    fit=np.poly1d(coef)
    x=np.linspace(1,4096,4096)
    x=fit(x)
    x=x.reshape((-1,1))#转置
    Spectrum=np.hstack((x,Spectrum0_copy))#横向叠加
    return Spectrum
...
def CutBackground(Spectrum0):
    ...
    for i in range(len(Spectrum)):
        for j in range(len(Background)-1):#前面已设置Background最少有3行,j必不为空
            if Background[j,0]<=Spectrum[i,0]<=Background[j+1,0]:#找背景对应点
                k=1#找到对应的背景点
                break
        if j==len(Background)-2 and k==0:#如果没找到对应点，且前一个也没有对应点,则起点+1
            DiffSBStart+=1
            k=0#k重新置零
            continue
...
def FindPeak(Spectrum0):
    ...
    for i in range(len(x)):
        ...
        if x[i]<10 and diff[i]<5:#标记要清除指定条件下的点
            z[i]=-1
        if x[i]>=0 and diff[i]<2:
            z[i]=-1
    j=0#j用于记录真峰的个数
        ...
        for k in range(i+1,len(x)):#将第u个峰与第k个峰比较
            for j in range(len(CutBackgroundSpectrum)):#找峰上的对应点
                ...
                if CutBackgroundSpectrum[j,0]==x[k]:
                    break
            if min(diff[u],diff[k])-min(CutBackgroundSpectrum[j0:j+1,1])<DiffThreshold and x[k]-x[u]<5:
...
def PeakAnalysis_and_Export(Spectrum0,row,col,worksheet1,worksheet2,style1,style2):
    ...
    Peak_Area[:,0]=CutDecimal(Peak_Area[:,0],2)#峰位置两位小数
    Peak_Area[:,2]=CutDecimal(Peak_Area[:,2],0)#峰面积保留整数
    Peak_Area[:,3]=CutDecimal(Peak_Area[:,3]*1000,0)#半峰宽乘1000，保留整数
...
def IdentifyPeak(PeakPos,PeakArea,worksheet1,style):
    #存放峰信息文件的地址
    filepath='F://桌面文件2019.2.11//Nuclear Physics Study//PyCode//PIXEPeaksIdentification.xlsx'
        ...
        for eo in range(len(ElementOrder)):
            ...
            for j in range(len(ElementPeakPosM)):
                if ElementPeakPosM[j]-ErrorRange<=PeakPos[i]<=ElementPeakPosM[j]+ErrorRange:
                    if (PeakOrderM[j]==1 or PeakOrderM[j]==3) and len(Element[i])==0:
                        #如果在范围内且是1或3级峰且未被填充过
                        Element[i]=ElementNameM[j]
                        OrderNumber[i]=PeakOrderM[j]
                        break
                    if PeakOrderM[j]==2:#二级峰搜索前面有没有一级峰
                        for k in range(len(Element)):
                            if Element[k]==ElementNameM[j] and OrderNumber[k]==1 and len(Element[i])==0:
                                Element[i]=ElementNameM[j]
                                isPeak=1
                                break
                        if isPeak==1:#如果找到了峰，就跳出循环
                            break
                    if PeakOrderM[j]==4:#四级峰搜索前面有没有三级峰
                        for k in range(len(Element)):
                            if Element[k]==ElementNameM[j] and OrderNumber[k]==3 and len(Element[i])==0:
                                Element[i]=ElementNameM[j]
                                isPeak=1
                                break
                        if isPeak==1:#如果找到了峰，就跳出循环
    ...
    for i in range(len(ElementWant2Know)):
        for j in range(len(Element)):
            if ElementWant2Know[i]==Element[j]:
